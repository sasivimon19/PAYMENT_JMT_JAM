        ...
        if($error && !$isCheckedAll && !$isChecked && !$onlyTrue){
            ...
            foreach ($result as $r) {
                ...
                if($count_port_error[0]->CP == 0){
                    ...
                    $this->EditData_model->Log_Update($port, $date, $cash_before, $cash_after, $idemp, $name, 'Import');
                }
                // $data = $this->Import_model->check_port_in_cashflow($port, $date);
                // if($data[0]->count_data == 0){
                //     $d = explode("-",$date);
...
//----------------------------------------------------------------------------------------------------------------//
    ...
    public function add_data() {
        ...
        $name = $this->session->userdata("NameEmp");
        $error = "";
        // $count_port = $this->Import_model->count_port();
        // $P = array();
        ...
        $this->Import_model->insert_data($number,$port,$cash,'1','1','1', iconv('UTF-8//IGNORE','TIS-620//IGNORE',$error),$name);   
        ...
        foreach($count_port as $p){
            ...
            if(array_count_values($P)[$PORT_EDIT] > 1){
                if(array_count_values($P)[$PORT_EDIT] <= 2){
                    ...
                    /////////////////////////////////////////////////////////////////////////////////////
                    $this->Import_model->edit_data($number, $port, $cash, iconv('UTF-8//IGNORE','TIS-620//IGNORE',$error),$name);
