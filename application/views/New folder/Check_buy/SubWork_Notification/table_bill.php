...
<?php  }else{  
     ...
     if($row->Row=='1'){
        $Pay_Start=$row->Date_pay;
     }
      if($row->Row==count($get_Picbill)){
        $Pay_End=$row->Date_pay;
     }
      ...
      <tr style="text-align: center">
       ...
       <?php   }else {  ?>
                ...
                <td><i class="fa fa-trash" aria-hidden="true" style="color: #136F63;cursor: pointer;"  onclick="funcDelete(AutoID=<?php echo $row->AutoID ?>,Insurance_price='<?php echo $Insurance_price ?>',PROSPECT_LIST_ID='<?php echo $row->PROSPECT_LIST_ID ?>',PaymentType='<?php echo $PaymentType ?>',payfirst='<?php echo $payfirst ?>')"></i></td>
        <?php  } ?> 
...
<?php 
    $start=0;
    $pageend =10;
    $where ="WHERE SaveBy ='".$Username."'  AND PROSPECT_LIST_ID='".$row->PROSPECT_LIST_ID."' ";
...
<div class="row">
 ...
 <?php if($checkStatus_Payslip == "1"){ ?>
      ...
      <?php }else{ ?>
              <button type="button" class="btn btn-primary btn-sm" id="Y" style="display:none"  data-toggle="modal" data-target="#modal-xl"  onclick="FuncConfirm(PeriodNumber=<?php echo $PeriodNumber ?>,TransactionID='<?php echo $TransactionID ?>',Date_Paymentfirst='<?php echo $Pay_Start ?>',Date_Payment='<?php echo $Pay_End ?>',Total_FirstPayment='<?php echo $Total_FirstPayment ?>',Installment='<?php echo $Installment ?>',PROSPECT_LIST_ID='<?php echo $PROSPECT_LIST_ID ?>')">ยืนยันการแจ้งงาน</button>
      <?php } ?>
    </div>
</div>
