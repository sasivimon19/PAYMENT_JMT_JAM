...
<form id="formfollow" name="formfollow" action="/action_page.php" style="background-color: #FFF;">
                  ...
                  <div id="show"></div>
                   <div class="tab" id="page1">
                      <div class="card-header">
                        ...
                        <?php $this->load->view('SubWork_NotificationEdit/Sub_personalEdit'); ?>
                    </div>
                    <div class="tab" id="page2">
                       <div class="card-header">
                          <h3 class="card-title">ข้อมูลรถยนต์</h3>
                       </div>
                        <?php $this->load->view('SubWork_NotificationEdit/Sub_datacarEdit'); ?>
                    </div>
                     <div class="tab" id="page3">
                       <div class="card-header">
                          <h3 class="card-title">ที่อยู่สำหรับส่งเอกสาร</h3>
                       </div>
                        <?php $this->load->view('SubWork_NotificationEdit/Sub_Address_send_docEdit'); ?>
                    </div>
                     <div class="tab" id="page4">
                       <div class="card-header">
                          <h3 class="card-title">ที่อยู่กรมธรรม์</h3>
                       </div>
                        <?php $this->load->view('SubWork_NotificationEdit/Sub_policy_addressEdit'); ?>
                    </div>
                     <div class="tab" id="page5">
                       <div class="card-header">
                          <h3 class="card-title">รายการการชำระเงิน</h3>
                       </div>
                        <?php $this->load->view('SubWork_NotificationEdit/Sub_listcashEdit'); ?>
                    </div>

                    <div style="height:25px;">
                        <div style="float:right;">
                          <button type="button" id="prevBtn1"  class="btnpn" onclick="nextPrev(-1)" style="height:35px;padding-bottom: 30px;">Previous</button>
                          <button type="button" id="nextBtn1"   class="btnpn" onclick="nextPrev(1)" style="background-color:#dc3545;height:35px;padding-bottom: 30px;">Next</button>
                        </div>
                    </div>
...
<script type="text/javascript">
    function funcSave() {
       ...
       document.getElementById("loadding").style.display = "block";
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('HomeInsurance/Save_Worknotification') ?>",
    ...
    function FuncSameAddress() {
        ...
        if (document.getElementById("same_address").checked == true) {
            ...
            document.getElementById("policy_zip").value = zip;
             ...
             $.ajax({
            type:"POST",
            url:"<?php echo site_url('HomeInsurance/selectbox')?>",
            data:datas
        }).done(function(json){ 
   ...
   var StatusButton=<?=$StatusButton?>;
    ...
    function showTab(n) {
        ...
        if (n == 0) {
         ...
         if (n == (x.length - 1)) {
            if(StatusButton == 5 || StatusButton == 6 || StatusButton== 7 || StatusButton == 8){
    ...
    function validateForm() {
       ...
       if(checkpage_1 == "block"){ // ข้อมูลผู้เอาประกัน
            ...
            if(datacar_paper == ""){
               ...
               document.getElementById("datacar_paper").focus();
            }else{
                document.getElementsByClassName("step")[currentTab].className += " finish";
                return valid;
            }
